 ...
 */
class WC_Gateway_Novalnet_Przelewy24 extends NN_Payment_Gateways {
	 ...
	 * @var string
	 */
	private $paygate_url      = 'https://payport.novalnet.de/globalbank_transfer';
	...
	public function __construct() {
		...
		add_action( 'woocommerce_api_response_novalnet_przelewy24', array( $this, 'check_novalnet_payment_response' ) );
	...
	public function get_order_status( $order_status ) {
		...
		if ( WC()->session->__isset( 'current_novalnet_payment' ) && WC()->session->current_novalnet_payment === $this->id && WC()->session->__isset( 'novalnet_przelewy24_order_status' ) ) {
			$order_status = WC()->session->novalnet_przelewy24_order_status;
		}
		return $order_status;
	}
	...
	public function check_novalnet_payment_response() {
		...
		if ( ! empty( $server_response['wc-api'] ) && 'response_' . $this->id === $server_response['wc-api'] ) {
			...
			WC()->session->set( 'novalnet_przelewy24_order_status', $this->settings ['order_success_status'] );
			if ( '86' === $server_response ['tid_status'] ) {
				WC()->session->set( 'novalnet_przelewy24_order_status', $this->settings ['pending_status'] );
			}
