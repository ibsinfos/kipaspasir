...
if($pid != "")
{
	$query="SELECT A.*, B.name, B.seo, C.username, C.rating, C.ratingcount, C.toprated, C.country FROM posts A, categories B, members C WHERE A.PID='".mysql_real_escape_string($pid)."' AND A.category=B.CATID AND A.USERID=C.USERID AND A.active='1'";
