...
if ($_SESSION['USERID'] != "" && $_SESSION['USERID'] >= 0 && is_numeric($_SESSION['USERID']))
{
	...
	$query1 = "SELECT count(*) as total from wants where active='1' AND USERID='".mysql_real_escape_string($UID)."' $addsql order by WID desc limit $config[maximum_results]";
	$query2 = "SELECT A.*, C.username from wants A, members C where A.active='1' AND A.USERID='".mysql_real_escape_string($UID)."' AND A.USERID=C.USERID order by $dby limit $pagingstart, $config[items_per_page]";
...
//TEMPLATES BEGIN
STemplate::assign('pagetitle',stripslashes($lang['511']));
