...
{	
	...
	if($cnt > 0)
	{
		...
		$query1 = "SELECT count(*) as total from posts A, members B where B.USERID='".mysql_real_escape_string($_SESSION['USERID'])."' AND A.USERID=B.USERID AND A.active!='3' order by A.PID desc limit $config[maximum_results]";
		$query2 = "SELECT A.*, B.seo, C.username from posts A, categories B, members C where A.category=B.CATID AND A.USERID=C.USERID AND C.USERID='".mysql_real_escape_string($_SESSION['USERID'])."' AND C.USERID=A.USERID AND A.active!='3' order by A.PID desc limit $pagingstart, $config[items_per_page]";
